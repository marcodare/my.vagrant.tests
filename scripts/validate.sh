...
while IFS= read -r -d '' file; do ruby -c "$file"; done < <(find . \( -name '*.rb' -o -name Vagrantfile -o -name Vagrant.start \) -not -path '*/.vagrant/*' -not -path '*/.venv/*' -print0)
...
if command -v vagrant >/dev/null; then
  for lab in */lab.json; do
    ...
    (cd "$(dirname "$lab")" && VAGRANT_VAGRANTFILE=Vagrant.start vagrant validate)
  done
else
